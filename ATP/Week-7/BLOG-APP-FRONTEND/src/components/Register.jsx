      ...
      <div className={formCard}>
        ...
        <form onSubmit={handleSubmit(onUserRegister)}>
          ...
          <div className={formGroup}>
            ...
            <input type="file" className={inputClass} accept="image/png, image/jpeg" {...register("profileImageUrl",{validate:{fileType:(files)=>{if(!files?.[0]) return true ; return ["image/png","image/jpeg"].includes(files[0].type) || "Only JPG/PNG ALLOWED"},fileSize:(files)=>{if(!files?.[0]) return true; return files[0].size <=2*1024*1024 || "Max is 2MB" }}})} onChange={(event)=>{let file=event.target.files[0]; if(file){setPreview(URL.createObjectURL(file))}}} />
